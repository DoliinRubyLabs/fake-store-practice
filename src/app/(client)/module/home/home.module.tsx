    ...
    <ContainerComponent>
      ...
      <div className={'mt-16 w-full space-y-12'}>
        {thisService.data?.sections?.map((data, index) => {
          const blockData = data?.section?.blocks?.[0]

          return (
            <Skeleton key={`${blockData?.id}-${index}`} isLoaded={!thisService.isLoading} className={'rounded-2xl'}>
              <Card
                className={
              ...
              >
                <CardBody className={'relative p-8 md:p-12'}>
                  ...
                  {blockData?.layout === 'image-top' || blockData?.layout === 'image-bottom' ? (
                    // Vertical layout (image top/bottom)
                    <div className={'space-y-8'}>
                      {blockData?.layout === 'image-top' && blockData?.image?.url && (
                        <div className={'w-full'}>
                          ...
                          >
                            ...
                            <Image
                              ...
                              alt={blockData?.image?.alt || blockData?.imageAlt || 'section image'}
                              fill
                              className={
                      ...
                      {blockData?.layout === 'image-bottom' && blockData?.image?.url && (
                        <div className={'w-full'}>
                          <div
                            className={
                              'group/image relative h-48 w-full overflow-hidden rounded-2xl ' +
                    ...
                    <div className={'grid gap-8 md:grid-cols-2 md:items-center'}>
                      <div
                        className={`space-y-6 ${blockData?.layout === 'image-left' ? 'order-2' : 'order-1'} ${
                          blockData?.alignment === 'center'
                            ? 'text-center'
                            : blockData?.alignment === 'right'
                              ? 'text-right'
                      ...
                      {blockData?.image?.url && (
                        <div className={`${blockData?.layout === 'image-left' ? 'order-1' : 'order-2'}`}>
                          <div
                            className={
                              'group/image relative h-48 w-full overflow-hidden rounded-2xl ' +
                              'bg-gradient-to-br from-gray-100 to-gray-200 transition-all duration-500' +
